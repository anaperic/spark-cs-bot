...
function Webhook(flint) {
  ...
  return function(req, res, next) {
    ...
    //ana
    flint.debug("Ana in webhook.js: " + name + "resource: " + resource + " event: " + event);
    //Ana removed - this was not working
 ...
 /*   if(name !== flint.webhook.name || (typeof flint.webhook.roomId !== 'undefined' && flint.webhook.roomId !== roomId)) {
  ...
  //ana
    ...
    if(resource === 'messages') {
      // membership created
      //ana
      console.log("Resaurce is messages");
      if(event === 'created') {
        console.log("Ana: Event created");
        flint.debug("Ana: in ebhook: message created: " + data.id);

        return flint.getMessage(data.id)
          .then(message => {
            flint.emit('messageCreated', message, flint.id);
            //Ana 
            flint.debug("Ana: in ebhook: message created: " + message);
